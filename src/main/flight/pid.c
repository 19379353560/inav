...
PG_RESET_TEMPLATE(pidProfile_t, pidProfile,
        ...
        .dterm_lpf2_hz = 250,   // Pre-diff LPF default 250Hz: ~0.6ms delay at 1kHz, replaced by SETTING_DTERM_LPF2_HZ_DEFAULT after build
...
static float FAST_CODE applyItermRelax(const int axis, const float iterm, float currentPidSetpoint, float itermErrorRate)
{
    if (itermRelax) {
        if (axis < FD_YAW || itermRelax == ITERM_RELAX_RPY) {
            ...
            const bool isDecreasingI =
                ((iterm > 0.0f) && (itermErrorRate < 0.0f)) ||
                ((iterm < 0.0f) && (itermErrorRate > 0.0f));

            // Betaflight-style unwind behaviour: keep full authority when the error is
            // reducing existing I-term so fast stick inputs do not leave stale I behind.
            if (isDecreasingI) {
...
static void FAST_CODE NOINLINE pidApplyMulticopterRateController(pidState_t *pidState, float dT, float dT_inv)
{
    ...
    const float rateError = rateTarget - pidState->gyroRate;
    float newPTerm = pTermProcess(pidState, rateError, dT);
...
#ifdef USE_ANTIGRAVITY
    if (pidState->axis != FD_YAW && iTermAntigravityGain > 1.0f) {
        float agSetpointAttenuator = fabsf(rateTarget) / 50.0f;
        agSetpointAttenuator = MAX(agSetpointAttenuator, 1.0f);

        const float antiGravityPBoost = 1.0f + constrainf(((iTermAntigravityGain - 1.0f) * 0.12f) / agSetpointAttenuator, 0.0f, 0.30f);
        newPTerm *= antiGravityPBoost;
    }
#endif
    ...
    float itermErrorRate = applyItermRelax(pidState->axis, pidState->errorGyroIf, rateTarget, rateError);
