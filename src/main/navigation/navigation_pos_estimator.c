...
#if defined(USE_BARO)
static float previousBaroAltitude = 0.0f;
static bool previousBaroAltitudeValid = false;
#endif
...
void onNewGPSData(void)
{
    ...
    if (sensors(SENSOR_GPS)
        ...
        if (posControl.gpsOrigin.valid) {
            fpVector3_t newGpsPos;

            /* Convert LLH position to local coordinates */
            geoConvertGeodeticToLocal(&newGpsPos, &posControl.gpsOrigin, &newLLH, GEO_ALT_ABSOLUTE);

            float rawGpsVelX = 0.0f;
            float rawGpsVelY = 0.0f;
            float rawGpsVelZ = 0.0f;

            /* If not the first update - calculate velocities */
            if (!isFirstGPSUpdate) {
                float dT = US2S(getGPSDeltaTimeFilter(currentTimeUs - lastGPSNewDataTime));
                const float predictedGpsPosX = posEstimator.gps.pos.x + posEstimator.gps.vel.x * dT;
                const float predictedGpsPosY = posEstimator.gps.pos.y + posEstimator.gps.vel.y * dT;
                const float gpsPredictionResidual = calc_length_pythagorean_2D(newGpsPos.x - predictedGpsPosX, newGpsPos.y - predictedGpsPosY);
                const float gpsGlitchAllowance = INAV_GPS_GLITCH_RADIUS + (0.5f * INAV_GPS_GLITCH_ACCEL * sq(dT));

                if (gpsPredictionResidual > gpsGlitchAllowance) {
                ...
                if (!ARMING_FLAG(SIMULATOR_MODE_SITL) && gpsSol.flags.validVelNE) {
                    rawGpsVelX = gpsSol.velNED[X];
                    rawGpsVelY = gpsSol.velNED[Y];
                }
                else {
                    rawGpsVelX = DISTANCE_BETWEEN_TWO_LONGITUDE_POINTS_AT_EQUATOR * (gpsSol.llh.lat - previousLat) / dT;
                    rawGpsVelY = gpsScaleLonDown * DISTANCE_BETWEEN_TWO_LONGITUDE_POINTS_AT_EQUATOR * (gpsSol.llh.lon - previousLon) / dT;
                }

                if (gpsSol.flags.validVelD) {
                    rawGpsVelZ = -gpsSol.velNED[Z];   // NEU
                }
                else {
                    rawGpsVelZ = (gpsSol.llh.alt - previousAlt) / dT;
                }

                posEstimator.gps.vel.x = pt1FilterApply4(&posEstimator.gps.velFilter[X], rawGpsVelX, INAV_GPS_VEL_AVERAGE_HZ, dT);
                posEstimator.gps.vel.y = pt1FilterApply4(&posEstimator.gps.velFilter[Y], rawGpsVelY, INAV_GPS_VEL_AVERAGE_HZ, dT);
                posEstimator.gps.vel.z = pt1FilterApply4(&posEstimator.gps.velFilter[Z], rawGpsVelZ, INAV_GPS_VEL_AVERAGE_HZ, dT);
            }
            else {
                rawGpsVelX = (!ARMING_FLAG(SIMULATOR_MODE_SITL) && gpsSol.flags.validVelNE) ? gpsSol.velNED[X] : 0.0f;
                rawGpsVelY = (!ARMING_FLAG(SIMULATOR_MODE_SITL) && gpsSol.flags.validVelNE) ? gpsSol.velNED[Y] : 0.0f;
                rawGpsVelZ = gpsSol.flags.validVelD ? -gpsSol.velNED[Z] : 0.0f;

                for (uint8_t axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
                    pt1FilterReset(&posEstimator.gps.velFilter[axis], axis == X ? rawGpsVelX : (axis == Y ? rawGpsVelY : rawGpsVelZ));
                }

                posEstimator.gps.vel.x = rawGpsVelX;
                posEstimator.gps.vel.y = rawGpsVelY;
                posEstimator.gps.vel.z = rawGpsVelZ;
                posEstimator.gps.updateDt = 0.0f;
            }

            posEstimator.gps.pos = newGpsPos;
...
void updatePositionEstimator_BaroTopic(timeUs_t currentTimeUs)
{
    ...
    if (sensors(SENSOR_BARO) && baroIsCalibrationComplete()) {
        ...
        if (shouldResetReferenceAltitude()) {
            ...
            previousBaroAltitudeValid = false;
            pt1FilterReset(&posEstimator.baro.rateFilter, 0.0f);
        }
        ...
        if (baroDtUs > 0 && baroDtUs <= MS2US(INAV_BARO_TIMEOUT_MS)) {
            ...
            // baro altitude rate
            if (previousBaroAltitudeValid) {
                const float rawBaroAltRate = (posEstimator.baro.alt - previousBaroAltitude) / baroDtSec;
                posEstimator.baro.baroAltRate = pt1FilterApply4(&posEstimator.baro.rateFilter, rawBaroAltRate, INAV_BARO_RATE_AVERAGE_HZ, baroDtSec);
            }
            else {
                posEstimator.baro.baroAltRate = 0.0f;
                pt1FilterReset(&posEstimator.baro.rateFilter, 0.0f);
                previousBaroAltitudeValid = true;
            }

            previousBaroAltitude = posEstimator.baro.alt;
...
static bool estimationCalculateCorrection_Z(estimationContext_t * ctx)
{
    ...
    if (ctx->newFlags & EST_BARO_VALID && wBaro) {
        if (posEstimator.baro.updateDt) {   // only update corrections once every sensor update
            ...
            float baroVelTrustScale = 1.0f;
            if (STATE(MULTIROTOR)) {
                const float dynamicVelocity = fabsf(posEstimator.est.vel.z) + (0.5f * posControl.actualState.velXY);
                const float dynamicVelocityScale = scaleRangef(constrainf(dynamicVelocity, 0.0f, 500.0f), 0.0f, 500.0f, 1.0f, 0.6f);
                const float tiltScale = scaleRangef(constrainf(calculateCosTiltAngle(), 0.6f, 1.0f), 1.0f, 0.6f, 1.0f, 0.5f);

                baroVelTrustScale = MIN(dynamicVelocityScale, tiltScale);
            }

            const float baroVelZResidual = isAirCushionEffectDetected ? 0.0f : wBaro * baroVelTrustScale * (posEstimator.baro.baroAltRate - posEstimator.est.vel.z);
    ...
    if (ctx->newFlags & EST_GPS_Z_VALID && (wGps || !(ctx->newFlags & EST_Z_VALID))) {
        if (posEstimator.gps.updateDt) {    // only update corrections once every sensor update
            ...
            if (!(ctx->newFlags & EST_Z_VALID)) {
                ...
                const float rawGpsAltResidual = posEstimator.gps.pos.z - posEstimator.est.pos.z;
                const float rawGpsVelZResidual = posEstimator.gps.vel.z - posEstimator.est.vel.z;
                const float gpsPositionScaler = scaleRangef(
                    bellCurve(fabsf(rawGpsAltResidual), MAX(posEstimator.gps.epv, INAV_GPS_DEFAULT_EPV)),
                    0.0f,
                    1.0f,
                    0.15f,
                    1.0f
                );
                const float gpsVelocityScaler = scaleRangef(
                    bellCurve(fabsf(rawGpsVelZResidual), MAX(150.0f, posEstimator.gps.epv * 0.5f)),
                    0.0f,
                    1.0f,
                    0.15f,
                    1.0f
                );
                const float gpsBiasScaler = MIN(gpsPositionScaler, gpsVelocityScaler);

                // Altitude
                const float gpsAltResidual = wGps * rawGpsAltResidual;
                const float gpsVelZResidual = wGps * rawGpsVelZResidual;
                const float w_z_gps_p = positionEstimationConfig()->w_z_gps_p * gpsPositionScaler;
                const float w_z_gps_v = positionEstimationConfig()->w_z_gps_v * sq(gpsVelocityScaler);

                ctx->estPosCorr.z += gpsAltResidual * w_z_gps_p * dT;
                ctx->estVelCorr.z += gpsVelZResidual * w_z_gps_v * dT;

                ctx->newEPV = updateEPE(ctx->newEPV, dT, MAX(posEstimator.gps.epv, fabsf(rawGpsAltResidual)), w_z_gps_p);

                // Accelerometer bias
                ctx->accBiasCorr.z += dT * gpsBiasScaler * (gpsAltResidual * sq(w_z_gps_p) + gpsVelZResidual * sq(w_z_gps_v));
            }
        }
...
static bool estimationCalculateCorrection_XY_GPS(estimationContext_t * ctx)
{
    if (ctx->newFlags & EST_GPS_XY_VALID) {
        ...
        if (!(ctx->newFlags & EST_XY_VALID)) {
            ...
            const float gpsVelResidualMag = calc_length_pythagorean_2D(gpsVelXResidual, gpsVelYResidual);

            const float gpsPositionScaler = scaleRangef(
                bellCurve(gpsPosResidualMag, MAX(posEstimator.gps.eph, INAV_GPS_ACCEPTANCE_EPE)),
                0.0f,
                1.0f,
                0.15f,
                1.0f
            ...
            const float gpsVelocityScaler = scaleRangef(
                bellCurve(gpsVelResidualMag, MAX(200.0f, posEstimator.gps.eph)),
                0.0f,
                1.0f,
                0.15f,
                1.0f
            ...
            const float w_xy_gps_p = positionEstimationConfig()->w_xy_gps_p * gpsPositionScaler;
            const float w_xy_gps_v = positionEstimationConfig()->w_xy_gps_v * sq(gpsVelocityScaler);
            ...
            // Accelerometer bias
            ctx->accBiasCorr.x = dT * gpsBiasScaler * (gpsPosXResidual * sq(w_xy_gps_p) + gpsVelXResidual * sq(w_xy_gps_v));
            ctx->accBiasCorr.y = dT * gpsBiasScaler * (gpsPosYResidual * sq(w_xy_gps_p) + gpsVelYResidual * sq(w_xy_gps_v));
...
void initializePositionEstimator(void)
{
    ...
    for (uint8_t axis = 0; axis < 3; axis++) {
        ...
        pt1FilterInit(&posEstimator.gps.velFilter[axis], INAV_GPS_VEL_AVERAGE_HZ, 0.0f);
    }

    pt1FilterInit(&posEstimator.baro.avgFilter, INAV_BARO_AVERAGE_HZ, 0.0f);
    pt1FilterInit(&posEstimator.baro.rateFilter, INAV_BARO_RATE_AVERAGE_HZ, 0.0f);
