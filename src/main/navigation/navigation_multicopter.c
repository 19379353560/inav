...
static pt1Filter_t hoverThrottleFilterState;
static float hoverThrottleEstimate = 0.0f;
static bool prepareForTakeoffOnReset = false;
static sqrt_controller_t alt_hold_sqrt_controller;

#define NAV_HOVER_THROTTLE_ADAPT_CUTOFF_HZ          0.2f
#define NAV_HOVER_THROTTLE_ADAPT_MAX_TARGET_VEL     25.0f
#define NAV_HOVER_THROTTLE_ADAPT_MAX_ACTUAL_VEL     50.0f
#define NAV_HOVER_THROTTLE_ADAPT_MAX_HORIZONTAL_VEL 150.0f
#define NAV_HOVER_THROTTLE_ADAPT_MIN_COS_TILT       0.965925813f
#define NAV_HOVER_THROTTLE_ADAPT_MARGIN             50
#define NAV_ALT_STOP_I_RELAX_VEL_CMS                120.0f
#define NAV_ALT_STOP_I_RELAX_TARGET_VEL_CMS         25.0f
#define NAV_ALT_DTERM_BOOST_START_VEL_CMS           250.0f
#define NAV_ALT_DTERM_BOOST_FULL_VEL_CMS            600.0f
#define NAV_ALT_DTERM_BOOST_MAX                     2.0f
#define NAV_POSHOLD_STOP_I_RELAX_VEL_CMS            80.0f
#define NAV_POSHOLD_STOP_TARGET_VEL_CMS             50.0f
#define NAV_POSHOLD_STOP_DTERM_BOOST_FULL_VEL_CMS   300.0f
#define NAV_POSHOLD_STOP_DTERM_BOOST_MAX            1.6f

static int16_t getHoverThrottleReference(void)
{
    if (hoverThrottleEstimate <= 0.0f) {
        hoverThrottleEstimate = currentBatteryProfile->nav.mc.hover_throttle;
    }

    return constrain(lrintf(hoverThrottleEstimate), getThrottleIdleValue() + 10, getMaxThrottle() - 10);
}

static int16_t constrainAltHoldThrottleReference(const int16_t throttleReference)
{
    return constrain(throttleReference,
        getThrottleIdleValue() + rcControlsConfig()->alt_hold_deadband + 10,
        getMaxThrottle() - rcControlsConfig()->alt_hold_deadband - 10);
}

static void updateAltHoldThrottleReference(void)
{
    if (navConfig()->mc.althold_throttle_type == MC_ALT_HOLD_HOVER) {
        altHoldThrottleRCZero = constrainAltHoldThrottleReference(getHoverThrottleReference());
    }
}

static void updateHoverThrottleEstimate(const int16_t throttleCommand, timeDelta_t deltaMicros)
{
    ...
    if (posControl.flags.isAdjustingAltitude ||
        ...
        ABS(posControl.desiredState.climbRateDemand) > 1 ||
        fabsf(posControl.desiredState.vel.z) > NAV_HOVER_THROTTLE_ADAPT_MAX_TARGET_VEL ||
        fabsf(actualState->vel.z) > NAV_HOVER_THROTTLE_ADAPT_MAX_ACTUAL_VEL ||
        posControl.actualState.velXY > NAV_HOVER_THROTTLE_ADAPT_MAX_HORIZONTAL_VEL ||
        calculateCosTiltAngle() < NAV_HOVER_THROTTLE_ADAPT_MIN_COS_TILT ||
        throttleCommand <= getThrottleIdleValue() + NAV_HOVER_THROTTLE_ADAPT_MARGIN ||
        throttleCommand >= getMaxThrottle() - NAV_HOVER_THROTTLE_ADAPT_MARGIN) {
        return;
    }

    hoverThrottleEstimate = pt1FilterApply4(&hoverThrottleFilterState, throttleCommand, NAV_HOVER_THROTTLE_ADAPT_CUTOFF_HZ, US2S(deltaMicros));
}
...
{
    const int16_t hoverThrottle = getHoverThrottleReference();
    const float verticalSpeed = fabsf(navGetCurrentActualPositionAndVelocity()->vel.z);
    const bool isStoppingForHold =
        !posControl.flags.isAdjustingAltitude &&
        fabsf(posControl.desiredState.vel.z) < NAV_ALT_STOP_I_RELAX_TARGET_VEL_CMS &&
        verticalSpeed > NAV_ALT_STOP_I_RELAX_VEL_CMS;
    const float dtermBoost = constrainf(
        scaleRangef(
            verticalSpeed,
            NAV_ALT_DTERM_BOOST_START_VEL_CMS,
            NAV_ALT_DTERM_BOOST_FULL_VEL_CMS,
            1.0f,
            NAV_ALT_DTERM_BOOST_MAX
        ),
        1.0f,
        NAV_ALT_DTERM_BOOST_MAX
    );
    ...
    const int16_t thrCorrectionMin = getThrottleIdleValue() - hoverThrottle;
    const int16_t thrCorrectionMax = getMaxThrottle() - hoverThrottle;

    const pidControllerFlags_e pidFlags = (posControl.flags.isAdjustingAltitude || isStoppingForHold) ? PID_SHRINK_INTEGRATOR : 0;
    float velocity_controller = navPidApply3(
    ...
    posControl.rcAdjustment[THROTTLE] = setDesiredThrottle(hoverThrottle + rcThrottleCorrection, false);
    updateHoverThrottleEstimate(posControl.rcAdjustment[THROTTLE], deltaMicros);
}
...
bool adjustMulticopterAltitudeFromRCInput(void)
{
    if (posControl.flags.isTerrainFollowEnabled) {
        ...
        updateAltHoldThrottleReference();
...
void setupMulticopterAltitudeController(void)
{
    ...
    const bool reinitializeHoverEstimate = !ARMING_FLAG(ARMED) || hoverThrottleEstimate <= 0.0f;

    if (reinitializeHoverEstimate) {
        hoverThrottleEstimate = currentBatteryProfile->nav.mc.hover_throttle;
        pt1FilterReset(&hoverThrottleFilterState, hoverThrottleEstimate);
    }
    ...
    altHoldThrottleRCZero = constrainAltHoldThrottleReference(altHoldThrottleRCZero);
...
void resetMulticopterAltitudeController(void)
{
    ...
    posControl.rcAdjustment[THROTTLE] = hoverThrottle;
...
static void updatePositionAccelController_MC(timeDelta_t deltaMicros, float maxAccelLimit, const float maxSpeed)
{
    ...
    const bool isStoppingForHold =
        !posControl.flags.isAdjustingPosition &&
        setpointXY < NAV_POSHOLD_STOP_TARGET_VEL_CMS &&
        posControl.actualState.velXY > NAV_POSHOLD_STOP_I_RELAX_VEL_CMS;
    const float stopDtermBoost = isStoppingForHold ?
        constrainf(
            scaleRangef(
                posControl.actualState.velXY,
                NAV_POSHOLD_STOP_I_RELAX_VEL_CMS,
                NAV_POSHOLD_STOP_DTERM_BOOST_FULL_VEL_CMS,
                1.0f,
                NAV_POSHOLD_STOP_DTERM_BOOST_MAX
            ),
            1.0f,
            NAV_POSHOLD_STOP_DTERM_BOOST_MAX
        ) : 1.0f;
    const float dtermScale = constrainf((1.0f - MIN(setpointScale, measurementScale)) * stopDtermBoost, 0.0f, NAV_POSHOLD_STOP_DTERM_BOOST_MAX);
    ...
    const pidControllerFlags_e pidFlags = (posControl.flags.isAdjustingPosition || isStoppingForHold) ? PID_SHRINK_INTEGRATOR : 0;
