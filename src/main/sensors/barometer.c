...
static bool baroNewSampleReady = false;
static timeMs_t baroLastUpdateMs = 0;

#define BARO_HEALTH_TIMEOUT_MS 500
...
bool baroIsNewSampleReady(void)
{
    return baroNewSampleReady;
}
...
bool baroIsHealthy(void)
{
    return sensors(SENSOR_BARO) &&
        baroLastUpdateMs != 0 &&
        (millis() - baroLastUpdateMs) <= BARO_HEALTH_TIMEOUT_MS;
}
