...
>
> **官方 PR：** [iNavFlight/inav#11464](https://github.com/iNavFlight/inav/pull/11464) is the active PR. [#11465](https://github.com/iNavFlight/inav/pull/11465) was closed to avoid duplication.
>
...
>
> **Technical note:** https://19379353560.github.io/dterm-prefilter-technical-note.html
...
| 类别 | 工作内容 | 核心收益 |
|---|---|---|
| 算法优化 | D 项预微分低通滤波 | 在差分前降低高频陀螺仪噪声；当前默认关闭，等待更多 Blackbox 飞测数据 |
...
在差分之前增加一级可选 PT1 低通预滤波：

```
原方案：陀螺仪原始值 ──→ [差分] ──→ [后置LPF] ──→ D输出
                          ↑ 噪声在此放大 ~9×

新方案：陀螺仪原始值 ──→ [可选前置PT1 LPF] ──→ [差分] ──→ [后置LPF] ──→ D输出
                                                  ↑ 先抑制高频输入噪声
```

当前 PR 中 `dterm_lpf2_hz` 默认值为 `0`，也就是禁用。这样已有 tune 不会改变，只有主动测试该设置的飞手才会启用。
...
| 测试项 | 说明 |
|---|---|
| Baseline | `dterm_lpf2_hz = 0`，保持现有行为 |
| Conservative pass | `dterm_lpf2_hz = 200`，用于噪声较明显的机架 |
| Less aggressive pass | `dterm_lpf2_hz = 250`，用于对比滤波强度 |
...
请尽量在同一机架、同一 PID/filter profile、同类电池和相似飞行动作下采集 Blackbox 日志。
...
- [D-term 技术说明](https://19379353560.github.io/dterm-prefilter-technical-note.html)
- [D-term 飞测指南](https://19379353560.github.io/dterm-flight-test-guide.html)
